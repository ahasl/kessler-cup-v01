...
@onready var _msg_label: Label = $Message/Panel/Margin/Label
...
func _ready() -> void:
	...
	if not EventBus.pending_message.is_empty():
		...
		_display(p.get("text", ""), p.get("level", "info"))
...
func _on_message(text: String, level: String) -> void:
	_display(text, level)


func _display(text: String, level: String) -> void:
	...
	_set_visible(true)
	# Long lines stay up longer so they're actually readable.
	_timer = clampf(MIN_SHOW_TIME + text.length() * SECONDS_PER_CHAR, MIN_SHOW_TIME, MAX_SHOW_TIME)
